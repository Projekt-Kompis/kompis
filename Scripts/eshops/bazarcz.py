...
def insertListing(listing):
	...
	cursor = connection.cursor()
	sqlIN = "INSERT INTO listing (store, item_condition, price, author, location, name, description, store_url, part_id, time_created) VALUES ('bazarcz', %s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE part_id = VALUES(part_id), description = VALUES(description), price = VALUES(price), is_invalid = 0"
...
def scrapeCPUs():
	...
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/procesory//{page}/")
		if not listings:
			return
...
def scrapeGPUs():
	...
	page = 1;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/graficke-karty/{page}/")
		if not listings:
			return
		for listing in listings:
			if isListingPresent(listing) and not doUpdate:
...
def scrapeMotherboards():
	...
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/zakladni-desky/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def scrapeRAMs():
	...
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/pameti/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def scrapeOSes():
	...
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/operacni-systemy/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def scrapeOptical():
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/cd-mechaniky/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def scrapeStorage():
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/pevne-disky/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def scrapeCases():
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/pocitacove-skrine/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def scrapePSUs():
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/dily-komponenty-zdroje/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def scrapeCoolers():
	...
	page = 0;
	while True:
		listings = scrapePage(f"https://www.bazar.cz/dily-komponenty-chladice/{page}/")
		if not listings:
			return
		for listing in listings:
			part = OrderedDict();
...
def updateAll():
	cursor = connection.cursor()
	cursor.execute("SELECT part_id, store_url FROM listing WHERE is_invalid = 0 AND store = 'bazarcz'")
